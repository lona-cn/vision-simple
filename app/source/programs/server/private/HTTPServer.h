...
constexpr std::string_view HTTPSERVER_OPT_KEY_INFER_FRAMEWORK{
    "infer_framework"};
...
struct HTTPServerOptions {
  ...
  const std::string& OptionOrPut(const std::string& key,
                                 const std::string& default_value);
  const std::string& OptionOrPut(std::string_view key,
                                 std::string_view default_value);
};

class HTTPServer {
 public:
  static HTTPServerResult<std::unique_ptr<HTTPServer>> Create(
  ...
  virtual HTTPServerResult<void> Run() noexcept = 0;
  virtual HTTPServerResult<void> StartAsync() noexcept = 0;
